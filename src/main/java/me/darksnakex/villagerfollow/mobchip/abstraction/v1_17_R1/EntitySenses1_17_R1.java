...
final class EntitySenses1_17_R1 implements EntitySenses {
   ...
   public EntitySenses1_17_R1(Mob m) {
      ...
      try {
         ...
         Map<SensorType<?>, Sensor<?>> sensors = (Map)sensorsF.get(this.nmsMob.getBehaviorController());
   ...
   private void save() {
      try {
         Field sensorsF = BehaviorController.class.getDeclaredField("e");
         sensorsF.setAccessible(true);
         sensorsF.set(this.nmsMob.getBehaviorController(), this.sensorsHandle);
   ...
   public List<me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?>> getSensors() {
      return (List)this.sensorsHandle.values().stream().map(ChipUtil1_17_R1::fromNMS).collect(Collectors.toList());
   }
   ...
   public void addSensor(@NotNull me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?> sensor) throws IllegalArgumentException {
      if (!(new ChipUtil1_17_R1()).existsSensor(sensor.getKey())) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(sensor.getKey()));
      } else {
         this.sensorsHandle.put(ChipUtil1_17_R1.toNMSType(sensor), ChipUtil1_17_R1.toNMS(sensor));
         this.save();
      }
   }
   ...
   public void removeSensor(@NotNull NamespacedKey key) {
      if (!(new ChipUtil1_17_R1()).existsSensor(key)) {
