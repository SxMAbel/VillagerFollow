...
final class EntityBody1_20_R1 implements EntityBody {
   private final EntityInsentient nmsMob;
   private final Mob m;

   public EntityBody1_20_R1(Mob m) {
      this.m = m;
      this.nmsMob = ChipUtil1_20_R1.toNMS(m);
   }

   private void update() {
      PacketPlayOutEntityMetadata packet = new PacketPlayOutEntityMetadata(this.nmsMob.af(), this.nmsMob.aj().b());
   ...
   public EntityBody.InteractionResult interact(@NotNull Player p, @Nullable EntityBody.InteractionHand hand) {
      ...
      switch(this.nmsMob.a(ChipUtil1_20_R1.toNMS(p), h)) {
      case a:
   ...
   public boolean isInvisibleTo(@Nullable Player p) {
      return this.nmsMob.d(ChipUtil1_20_R1.toNMS(p));
   }

   @NotNull
   public EntityBody.InteractionHand getMainHand() {
      return this.nmsMob.fh() == EnumMainHand.a ? EntityBody.InteractionHand.OFF_HAND : EntityBody.InteractionHand.MAIN_HAND;
   }
   ...
   public boolean isInCombat() {
      try {
         ...
         return inCombatF.getBoolean(this.nmsMob.bQ);
   ...
   public boolean isMoving() {
      double x = this.nmsMob.dn() - this.nmsMob.J;
      double z = this.nmsMob.dt() - this.nmsMob.L;
   ...
   public void playAnimation(@NotNull EntityAnimation anim) {
      ...
      case CRITICAL_DAMAGE:
         ...
         var3 = ChipUtil1_20_R1.fromNMS(this.nmsMob).getWorld().getPlayers().iterator();

         while(var3.hasNext()) {
            p = (Player)var3.next();
            ChipUtil1_20_R1.toNMS(p).c.a(pkt);
         }
   ...
   public boolean isInBubbleColumn() {
      return this.nmsMob.cH().a_(this.nmsMob.di()).a(Blocks.nd);
   }

   public boolean isInvulnerableTo(@Nullable DamageCause cause) {
      return this.nmsMob.b(ChipUtil1_20_R1.toNMS((DamageCause)cause, (Entity)this.m));
   }
   ...
   public boolean isPushableBy(@Nullable Entity entity) {
      return IEntitySelector.a(ChipUtil1_20_R1.toNMS(entity)).test(ChipUtil1_20_R1.toNMS(entity));
   }
   ...
   public void setYaw(float rotation) {
      this.nmsMob.a_(EntityBody.normalizeRotation(rotation));
   }
   ...
   public void setPitch(float rotation) {
      this.nmsMob.b_(EntityBody.normalizeRotation(rotation));
   }

   public float getMaxUpStep() {
      return this.nmsMob.dC();
   }

   public void setMaxUpStep(float maxUpStep) {
      this.nmsMob.r(maxUpStep);
   }
   ...
   public void eat(@NotNull ItemStack item) {
      this.nmsMob.a(ChipUtil1_20_R1.toNMS(this.m.getWorld()), ChipUtil1_20_R1.toNMS(item));
   }

   public void setRotation(float yaw, float pitch) {
      try {
         if (this.m instanceof Slime) {
            ControllerMove moveControl = this.nmsMob.G();
