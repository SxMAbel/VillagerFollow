...
final class EntitySenses1_20_R2 implements EntitySenses {
   ...
   public EntitySenses1_20_R2(Mob m) {
      ...
      try {
         ...
         Map<SensorType<?>, Sensor<?>> sensors = (Map)sensorsF.get(this.nmsMob.dN());
   ...
   private void save() {
      try {
         Field sensorsF = BehaviorController.class.getDeclaredField("e");
         sensorsF.setAccessible(true);
         sensorsF.set(this.nmsMob.dN(), this.sensorsHandle);
   ...
   public List<me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?>> getSensors() {
      return (List)this.sensorsHandle.values().stream().map(ChipUtil1_20_R2::fromNMS).collect(Collectors.toList());
   }
   ...
   public void addSensor(@NotNull me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?> sensor) throws IllegalArgumentException {
      if (!(new ChipUtil1_20_R2()).existsSensor(sensor.getKey())) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(sensor.getKey()));
      } else {
         this.sensorsHandle.put(ChipUtil1_20_R2.toNMSType(sensor), ChipUtil1_20_R2.toNMS(sensor));
         this.save();
      }
   }
   ...
   public void removeSensor(@NotNull me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?> sensor) {
      if (!(new ChipUtil1_20_R2()).existsSensor(sensor.getKey())) {
         ...
         SensorType<?> type = ChipUtil1_20_R2.toNMSType(sensor);
         MinecraftKey key = BuiltInRegistries.D.b(type);
         ...
         while(it.hasNext()) {
            ...
            MinecraftKey currentKey = BuiltInRegistries.D.b(currentType);
   ...
   public void removeSensor(@NotNull NamespacedKey key) {
      if (!(new ChipUtil1_20_R2()).existsSensor(key)) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(key));
      } else {
         MinecraftKey keyH = ChipUtil1_20_R2.toNMS(key);
         Iterator it = this.sensorsHandle.entrySet().iterator();
   ...
   public boolean hasSensor(@NotNull NamespacedKey key) {
      ...
      while(var3.hasNext()) {
         ...
         MinecraftKey currentKey = BuiltInRegistries.D.b(t);
         if (ChipUtil1_20_R2.toNMS(key).equals(currentKey)) {
            b.set(true);
            break;
         }
      }
