...
final class EntitySenses1_21_R4 implements EntitySenses {
   ...
   public EntitySenses1_21_R4(Mob m) {
      ...
      try {
         ...
         Map<SensorType<?>, Sensor<?>> sensors = (Map)sensorsF.get(this.nmsMob.ec());
   ...
   public List<me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?>> getSensors() {
      return (List)this.sensorsHandle.values().stream().map(ChipUtil1_21_R4::fromNMS).collect(Collectors.toList());
   }
   ...
   public void addSensor(@NotNull me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?> sensor) throws IllegalArgumentException {
      if (!(new ChipUtil1_21_R4()).existsSensor(sensor.getKey())) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(sensor.getKey()));
      } else {
         this.sensorsHandle.put(ChipUtil1_21_R4.toNMSType(sensor), ChipUtil1_21_R4.toNMS(sensor));
         this.save();
      }
   }
   ...
   public void removeSensor(@NotNull NamespacedKey key) {
      if (!(new ChipUtil1_21_R4()).existsSensor(key)) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(key));
      } else {
         MinecraftKey keyH = ChipUtil1_21_R4.toNMS(key);
   ...
   public boolean hasSensor(@NotNull NamespacedKey key) {
      ...
      while(var3.hasNext()) {
         ...
         if (ChipUtil1_21_R4.toNMS(key).equals(currentKey)) {
            b.set(true);
            break;
         }
      }
