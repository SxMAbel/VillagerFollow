...
final class EntitySenses1_21_R6 implements EntitySenses {
   ...
   public EntitySenses1_21_R6(Mob m) {
      ...
      try {
         ...
         Map<SensorType<?>, Sensor<?>> sensors = (Map)sensorsF.get(this.nmsMob.eq());
   ...
   private void save() {
      try {
         Field sensorsF = BehaviorController.class.getDeclaredField("e");
         sensorsF.setAccessible(true);
         sensorsF.set(this.nmsMob.eq(), this.sensorsHandle);
   ...
   public List<me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?>> getSensors() {
      return (List)this.sensorsHandle.values().stream().map(ChipUtil1_21_R6::fromNMS).collect(Collectors.toList());
   }
   ...
   public void addSensor(@NotNull me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?> sensor) throws IllegalArgumentException {
      if (!(new ChipUtil1_21_R6()).existsSensor(sensor.getKey())) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(sensor.getKey()));
      } else {
         this.sensorsHandle.put(ChipUtil1_21_R6.toNMSType(sensor), ChipUtil1_21_R6.toNMS(sensor));
         this.save();
      }
   }
   ...
   public void removeSensor(@NotNull NamespacedKey key) {
      if (!(new ChipUtil1_21_R6()).existsSensor(key)) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(key));
      } else {
         MinecraftKey keyH = ChipUtil1_21_R6.toNMS(key);
         ...
         while(it.hasNext()) {
            ...
            MinecraftKey currentKey = BuiltInRegistries.B.b(currentType);
   ...
   public boolean hasSensor(@NotNull NamespacedKey key) {
      ...
      while(var3.hasNext()) {
         ...
         MinecraftKey currentKey = BuiltInRegistries.B.b(t);
         if (ChipUtil1_21_R6.toNMS(key).equals(currentKey)) {
            b.set(true);
            break;
         }
      }
