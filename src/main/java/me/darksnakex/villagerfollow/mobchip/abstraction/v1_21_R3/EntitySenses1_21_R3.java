...
final class EntitySenses1_21_R3 implements EntitySenses {
   ...
   public EntitySenses1_21_R3(Mob m) {
      ...
      try {
         ...
         Map<SensorType<?>, Sensor<?>> sensors = (Map)sensorsF.get(this.nmsMob.eb());
   ...
   private void save() {
      try {
         Field sensorsF = BehaviorController.class.getDeclaredField("e");
         sensorsF.setAccessible(true);
         sensorsF.set(this.nmsMob.eb(), this.sensorsHandle);
   ...
   public List<me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?>> getSensors() {
      return (List)this.sensorsHandle.values().stream().map(ChipUtil1_21_R3::fromNMS).collect(Collectors.toList());
   }
   ...
   public void addSensor(@NotNull me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?> sensor) throws IllegalArgumentException {
      if (!(new ChipUtil1_21_R3()).existsSensor(sensor.getKey())) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(sensor.getKey()));
      } else {
         this.sensorsHandle.put(ChipUtil1_21_R3.toNMSType(sensor), ChipUtil1_21_R3.toNMS(sensor));
         this.save();
      }
   }
   ...
   public void removeSensor(@NotNull NamespacedKey key) {
      if (!(new ChipUtil1_21_R3()).existsSensor(key)) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(key));
      } else {
         MinecraftKey keyH = ChipUtil1_21_R3.toNMS(key);
   ...
   public boolean hasSensor(@NotNull NamespacedKey key) {
      ...
      while(var3.hasNext()) {
         ...
         if (ChipUtil1_21_R3.toNMS(key).equals(currentKey)) {
            b.set(true);
            break;
         }
      }
