...
final class SensorDefault1_19_R1 implements Sensor<LivingEntity> {
   private final net.minecraft.world.entity.ai.sensing.Sensor<?> handle;

   public SensorDefault1_19_R1(net.minecraft.world.entity.ai.sensing.Sensor<?> handle) {
      this.handle = handle;
   }
   ...
   public List<Memory<?>> required() {
      return (List)this.handle.a().stream().map(ChipUtil1_19_R1::fromNMS).collect(Collectors.toList());
   }
   ...
   public void run(@NotNull World w, LivingEntity entity) {
      try {
         ...
         doTick.invoke(this.handle, ChipUtil1_19_R1.toNMS(w), ChipUtil1_19_R1.toNMS(entity));
   ...
   public NamespacedKey getKey() {
      ...
      IRegistry.as.r().filter((s) -> {
         return s.a().equals(this.handle);
      }).findFirst().ifPresent((s) -> {
         key.set(ChipUtil1_19_R1.fromNMS(IRegistry.as.b(s)));
      });
