...
final class SensorDefault1_17_R1 implements Sensor<LivingEntity> {
   ...
   public List<Memory<?>> required() {
      return (List)this.handle.a().stream().map(ChipUtil1_17_R1::fromNMS).collect(Collectors.toList());
   }
   ...
   public void run(@NotNull World w, LivingEntity entity) {
      try {
         ...
         doTick.invoke(this.handle, ChipUtil1_17_R1.toNMS(w), ChipUtil1_17_R1.toNMS(entity));
   ...
   public NamespacedKey getKey() {
      ...
      IRegistry.as.g().filter((s) -> {
         return s.a().equals(this.handle);
      }).findFirst().ifPresent((s) -> {
         key.set(ChipUtil1_17_R1.fromNMS(IRegistry.as.getKey(s)));
      });
