...
final class EntityGossipContainer1_21_R5 implements EntityGossipContainer {
   private final Reputation handle;
   private final Villager entity;

   public EntityGossipContainer1_21_R5(Villager v) {
      this.entity = v;
      this.handle = ((CraftVillager)v).getHandle().hc();
   }
   ...
   public int getReputation(@NotNull Entity en, @Nullable GossipType... types) throws IllegalArgumentException {
      return this.handle.a(en.getUniqueId(), (g) -> {
         return Arrays.asList(types).contains(ChipUtil1_21_R5.fromNMS(g));
      });
   }

   public void put(@NotNull Entity en, @NotNull GossipType type, int maxCap) throws IllegalArgumentException {
      this.handle.add(en.getUniqueId(), ChipUtil1_21_R5.toNMS(type), maxCap, ReputationEvent.UNSPECIFIED);
   }

   public void remove(@NotNull Entity en, @NotNull GossipType type) throws IllegalArgumentException {
      this.handle.remove(en.getUniqueId(), ChipUtil1_21_R5.toNMS(type), ReputationEvent.UNSPECIFIED);
   }

   public void removeAll(@NotNull GossipType type) throws IllegalArgumentException {
      this.handle.remove(ChipUtil1_21_R5.toNMS(type), ReputationEvent.UNSPECIFIED);
   }
}
