...
final class SensorDefault1_18_R1 implements Sensor<LivingEntity> {
   ...
   public List<Memory<?>> required() {
      return (List)this.handle.a().stream().map(ChipUtil1_18_R1::fromNMS).collect(Collectors.toList());
   }
   ...
   public void run(@NotNull World w, LivingEntity entity) {
      try {
         ...
         doTick.invoke(this.handle, ChipUtil1_18_R1.toNMS(w), ChipUtil1_18_R1.toNMS(entity));
   ...
   public NamespacedKey getKey() {
      ...
      IRegistry.at.j().filter((s) -> {
         return s.a().equals(this.handle);
      }).findFirst().ifPresent((s) -> {
         key.set(ChipUtil1_18_R1.fromNMS(IRegistry.at.b(s)));
      });
