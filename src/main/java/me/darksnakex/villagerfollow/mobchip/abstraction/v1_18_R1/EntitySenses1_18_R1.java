...
final class EntitySenses1_18_R1 implements EntitySenses {
   ...
   public EntitySenses1_18_R1(Mob m) {
      this.m = m;
   ...
   public void removeSensor(@NotNull me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?> sensor) {
      if (!(new ChipUtil1_18_R1()).existsSensor(sensor.getKey())) {
         ...
         SensorType<?> type = ChipUtil1_18_R1.toNMSType(sensor);
         MinecraftKey key = IRegistry.at.b(type);
         ...
         while(it.hasNext()) {
            ...
            MinecraftKey currentKey = IRegistry.at.b(currentType);
   ...
   public void removeSensor(@NotNull NamespacedKey key) {
      if (!(new ChipUtil1_18_R1()).existsSensor(key)) {
         throw new IllegalArgumentException("Unregistered Sensor: " + String.valueOf(key));
      } else {
         MinecraftKey keyH = ChipUtil1_18_R1.toNMS(key);
         Iterator it = this.sensorsHandle.entrySet().iterator();
   ...
   public boolean hasSensor(@NotNull NamespacedKey key) {
      ...
      while(var3.hasNext()) {
         ...
         MinecraftKey currentKey = IRegistry.at.b(t);
         if (ChipUtil1_18_R1.toNMS(key).equals(currentKey)) {
            b.set(true);
            break;
         }
      }
