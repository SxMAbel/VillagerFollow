...
final class EntitySenses1_21_R2 implements EntitySenses {
   private final Mob m;
   ...
   public List<me.darksnakex.villagerfollow.mobchip.ai.sensing.Sensor<?>> getSensors() {
      return (List)this.sensorsHandle.values().stream().map(ChipUtil1_21_R2::fromNMS).collect(Collectors.toList());
   }
   ...
   public boolean hasSensor(@NotNull NamespacedKey key) {
      ...
      while(var3.hasNext()) {
         ...
         if (ChipUtil1_21_R2.toNMS(key).equals(currentKey)) {
            b.set(true);
            break;
         }
      }
