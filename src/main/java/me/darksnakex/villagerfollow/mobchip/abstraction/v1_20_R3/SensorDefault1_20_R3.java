...
final class SensorDefault1_20_R3 implements Sensor<LivingEntity> {
   private final net.minecraft.world.entity.ai.sensing.Sensor<?> handle;

   public SensorDefault1_20_R3(net.minecraft.world.entity.ai.sensing.Sensor<?> handle) {
      this.handle = handle;
   }
   ...
   public List<Memory<?>> required() {
      return (List)this.handle.a().stream().map(ChipUtil1_20_R3::fromNMS).collect(Collectors.toList());
   }
   ...
   public void run(@NotNull World w, LivingEntity entity) {
      try {
         ...
         doTick.invoke(this.handle, ChipUtil1_20_R3.toNMS(w), ChipUtil1_20_R3.toNMS(entity));
   ...
   public NamespacedKey getKey() {
      ...
      BuiltInRegistries.C.s().filter((s) -> {
         ...
         key.set(ChipUtil1_20_R3.fromNMS(BuiltInRegistries.C.b(s)));
      });
