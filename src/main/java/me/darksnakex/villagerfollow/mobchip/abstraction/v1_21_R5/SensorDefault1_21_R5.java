...
final class SensorDefault1_21_R5 implements Sensor<LivingEntity> {
   private final net.minecraft.world.entity.ai.sensing.Sensor<?> handle;

   public SensorDefault1_21_R5(net.minecraft.world.entity.ai.sensing.Sensor<?> handle) {
      this.handle = handle;
   }
   ...
   public List<Memory<?>> required() {
      return (List)this.handle.a().stream().map(ChipUtil1_21_R5::fromNMS).collect(Collectors.toList());
   }
   ...
   public void run(@NotNull World w, LivingEntity entity) {
      try {
         ...
         doTick.invoke(this.handle, ChipUtil1_21_R5.toNMS(w), ChipUtil1_21_R5.toNMS(entity));
   ...
   public NamespacedKey getKey() {
      ...
      BuiltInRegistries.A.s().filter((s) -> {
         ...
         key.set(ChipUtil1_21_R5.fromNMS(BuiltInRegistries.A.b(s)));
      });
